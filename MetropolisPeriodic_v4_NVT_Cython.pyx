"""
Created on Sat Apr 12 13:33:23 2014
PHYS 780, Assignment 9
Nick Crump
"""
...
# v4 new calculations & running averages, radial distr function g(r)
...
cpdef SimRun():
    ...
    cdef np.ndarray[dtype=double,ndim=1] GRbins,GRfunc
    cdef np.ndarray[dtype=double,ndim=1] RadArr,histBin
    cdef np.ndarray[dtype=double,ndim=1] LJRow,LJCol
    cdef np.ndarray[dtype=double,ndim=2] LJm
    cdef np.ndarray[dtype=long,ndim=1] histNum
    ...
    for k in range(nRho):
        ...
        for t in range(nTmp):
            ...
            RadArr = np.zeros(N,dtype=float)           # radius array for g(r)
            GRfunc = np.zeros(Nbins,dtype=float)       # radial distr func g(r)
                ...
                if run >= hlfRun:
                    ...
                    for i in range(N):
                        for j in range(N):
                            ...
                            imgR = (dX*dX + dY*dY + dZ*dZ)**0.5
                            RadArr[j] = imgR
                        histNum,histBin = np.histogram(RadArr,bins=GRbins+0.05)
                        GRfunc = GRfunc + histNum
                #----------------------------------------------------
            ...
            GRfunc = GRfunc / (hlfRun*N*Rho*4*pi*0.05*GRbins[0:Nbins]**2)
            GR.append(GRfunc)

            # calculate Cv from variance of energy
            Cv = varU / T**2

            print 'Rho = %1.2f T = %1.2f <U> = %1.2f Ucorr = %.2f varU = %1.2f accP = %1.2f Cv = %1.2f' \
            % (Rho,T,aveU,Ucorr,varU,accP,Cv)
...
#            #----------------------------------------------------
#
#        # plot radial distribution function g(r) vs radius
#        title3 = '$g(r)$, '+r'$\rho$ = '+np.str(Rho)+', NVT'
#        fname3 = np.str(N)+'atom_RadDistFunc_Rho'+np.str(Rho)+'_NVT.png'
#        plt.figure()
#        plt.plot(GRbins[0:Nbins],GR[0],'b-',label='T=0.8')
#        plt.plot(GRbins[0:Nbins],GR[1],'r-',label='T=2.0')
...
#        title4 = r'$\rho$ = '+np.str(Rho)+', NVT'
#        fname4 = np.str(N)+'atom_energy_var_acc_Rho'+np.str(Rho)+'_NVT.png'
#        plt.figure()
#        plt.subplot(311)
